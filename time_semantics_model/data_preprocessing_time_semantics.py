...
def convert_note_to_time_annot(
    ...
    if if_aug_time == False:
        beat_aug_time = 1/(sampling_rate*2)
        downbeat_aug_time = 1/(sampling_rate*2)
        phrase_aug_time = 1/(sampling_rate*2)
    
    else:
        beat_aug_time = beat_aug_time
        downbeat_aug_time = downbeat_aug_time
        phrase_aug_time = phrase_aug_time
    
    print('beat augment time:', beat_aug_time, downbeat_aug_time, phrase_aug_time)
...
def get_motion_data(
    ...
    print('motion start frame, end frame:', start_frame, end_frame)
...
def get_motion_velocity(data_arr, motion_sampling_rate = 120):
    ...
    arr_diff = (data_arr - data_roll) * motion_sampling_rate / 10
    arr_diff = arr_diff[1:, :]
    last = arr_diff[-1, :]
...
def get_audio_data(
    ...
    print('audio start inx:', start_inx, end_inx)
...
def get_snippet(
    ...
    print('get snippet num:', len(snippet_list))
...
def get_song_data_label_snippet_list(
    ...
    # check_nan(song_motion_data)
    # print('motion data')
    # for inx in range(3):
    #     print(inx, song_motion_data[inx, :])
    
    
    # get motion velocity
    song_velocity_data = get_motion_velocity(song_motion_data)
    ...
    song_snippet_list = get_snippet(
        ...
        song_velocity_data, 
        song_annot_list,
        song_name + '_' + perf_name + '_' + trial_name,
...
if __name__ == '__main__':
    ...
    body_marker_num = 22 # 30 for motion generation, 22 for time/expression semantics
    instrument_marker_num = 0 # 4 for motion generation, 0 for time/expression semantics
    snippet_time_window = 16
    snippet_hop_size = 2
